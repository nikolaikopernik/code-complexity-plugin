...
class KtLanguageInfoProvider(override val language: Language = KotlinLanguage.INSTANCE) : LanguageInfoProvider {
    ...
    internal class KtLanguageVisitor(private val sink: ComplexitySink) : ElementVisitor() {
        override fun processElement(element: PsiElement) {
            when (element) {
                is KtWhileExpression -> sink.increaseComplexityAndNesting(LOOP_WHILE)
                is KtDoWhileExpression -> sink.increaseComplexityAndNesting(LOOP_WHILE)
                is KtWhenExpression -> sink.increaseComplexityAndNesting(SWITCH)
                ...
                is KtForExpression -> sink.increaseComplexityAndNesting(LOOP_FOR)
                is KtCatchClause -> sink.increaseComplexityAndNesting(CATCH)
                is KtBreakExpression -> if (element.labelQualifier != null) sink.increaseComplexity(BREAK)
                is KtContinueExpression -> if (element.labelQualifier != null) sink.increaseComplexity(CONTINUE)
                ...
                is KtElement -> if (isRecursiveCall(element)) sink.increaseComplexity(RECURSION)
            }
        }
...
private fun KtToken.toPointType(): PointType =
    when (this) {
        KtTokens.ANDAND -> LOGICAL_AND
        KtTokens.OROR -> LOGICAL_OR
        else -> UNKNOWN
    }
